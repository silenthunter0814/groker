...
\begin{table}[h]
    \centering
    \caption{常见曲线的参数方程（极简版）}
    \label{tab:parametric_equations_minimal}
    \begin{tabular}{ccccc}
        \textbf{曲线类型} & \textbf{普通方程} & \textbf{参数方程} & \textbf{参数 $t$ 的范围} & \textbf{备注} \\
        \midrule % 仅在表头下方使用这条中等粗细横线
        直线 & $y - y_0 = m(x - x_0)$ & $\begin{cases} x = x_0 + a t \\ y = y_0 + b t \end{cases}$ & $t \in (-\infty, \infty)$ & $m = b/a$ \\
        圆 & $(x-h)^2 + (y-k)^2 = r^2$ & $\begin{cases} x = h + r \cos t \\ y = k + r \sin t \end{cases}$ & $t \in [0, 2\pi)$ & $t$ 表示圆心角 \\
        椭圆 & $\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1$ & $\begin{cases} x = a \cos t \\ y = b \sin t \end{cases}$ & $t \in [0, 2\pi)$ & $t$ 为辅助角 \\
        抛物线 & $y^2 = 2px$ & $\begin{cases} x = \frac{p}{2} t^2 \\ y = p t \end{cases}$ & $t \in (-\infty, \infty)$ & $p$ 为焦距 \\
        % 没有 \bottomrule
    \end{tabular}
\end{table}
