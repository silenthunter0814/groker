...
从格洛克代数空间的角度看，自然常数是一个未解析的无穷大无穷小计算表达式。
...
\begin{example} 计算极限
    \begin{align*}
        ...
        &= \left(1 + \frac{2}{2\infty}\right)^{2\infty} \qquad (\infty \gets 2\infty) \\
        &= \left[\left(1 + \frac{1}{\infty}\right)^\infty \right]^2 \\
        &= e^2 \\
        \\
        \lim_{x \to 0}\ln(1+x) &= \ln(1+0) = 0
    \end{align*}
\end{example}

\medskip

\begin{example} 计算极限 $\lim_{x \to 0}\frac{e^x - 1}{x}$。
    \begin{align*}
...
结合例2.3.2 的结果，并巧妙的利用无穷小的等效重定义，我们可以计算一般指数函数的极限。

\begin{example} 计算极限 $\lim_{x \to 0}\frac{a^x - 1}{x}$。
    \begin{align*}
        a^x = e^{\ln a^x} &= e^{x\ln a} \\
        \lim_{x \to 0}\frac{a^x - 1}{x} &= \frac{a^\epsilon - 1}{\epsilon} = \frac{e^{\epsilon\ln a} - 1}{\epsilon} \\
        &= \frac{e^\epsilon - 1}{\epsilon/\ln a} \qquad (\epsilon \gets \epsilon/\ln a) \\
        &= \ln a \cdot \frac{e^\epsilon - 1}{\epsilon} \\
        &= \ln a
    \end{align*}
\end{example}
...
\begin{example} 计算极限 $\lim_{x \to 0}\frac{a^x - 1}{x}$。
    \begin{align*}
        ...
        y &= \frac{a^\epsilon - 1}{\epsilon} \\
        1 + y\epsilon &= a^\epsilon \\
        \ln (1 + y\epsilon) &= \ln a^\epsilon \\
        y \cdot \epsilon &= \ln a \cdot \epsilon \\
        y &= \ln a \\
