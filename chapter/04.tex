...
我们在 $x_0$ 的最小微区间 $[x_0-\epsilon,\;x_0+\epsilon]$ 进行极值定义和判断，彻底消除了传统极值定义的模糊化表述。
...
textbf{极值与最值（全局极值）的区别}
极值是一个局部性的概念，而最值是全局性的概念。

$$\begin{array}{|c|c|c|}
    \textbf{特征} & \textbf{极值（局部极值）} & \textbf{最值（全局极值/绝对极值）} \\
    \hline
    \textbf{定义范围} & \text{在} x_0 \text{点的某个邻域} I \text{内} & \text{在函数的整个定义域} D \text{或给定区间} [a, b] \text{内} \\
    \textbf{存在个数} & \text{可以有多个局部极大值或极小值} & \text{最多只有一个最大值和一个最小值} \\
    \textbf{大小比较} & \begin{aligned} & \text{局部极大值不一定大于局部极小值} \\ & (\text{仅在其局部区域内是最大或最小}) \end{aligned} & \begin{aligned} & \text{最大值必然是所有函数值的最大} \\ & \text{最小值必然是所有函数值的最小} \end{aligned} \\
    \textbf{联系} & \text{最值如果不在端点处取得，必然是相应的极值} & \text{最值是全局范围内的最大值和最小值} \\
    \end{array}$$