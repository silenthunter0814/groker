...
分部积分法的步骤
应用分部积分法的关键在于对被积函数 $f(x)$ 进行合理的分解，即选择 $u$ 和 $dv$。
（1）分解被积函数：将被积函数 $f(x) \,dx$ 分解为两部分：$u$ 和 $dv$。
（2）目标： 使 $\int v \,du$ 比 $\int u \,dv$ 更容易计算。
（3）求 $du$ 和 $v$：对 $u$ 求微分，得到 $du$。对 $dv$ 求积分，得到 $v$（通常忽略常数 $C$，因为最终的 $C$ 会加在最后的结果中）。
（4）应用公式：将被积函数、 $u$、 $v$ 和 $du$ 代入分部积分公式：$$\int u dv = uv - \int v du$$
（5）求解新的积分：计算等式右侧的积分 $\int v du$。
