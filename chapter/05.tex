...
\begin{array}{ll}
    ...
    \text{II. 重复的线性因子 } (ax+b)^n &\quad \frac{A_1}{ax+b} + \frac{A_2}{(ax+b)^2} + \dots + \frac{A_n}{(ax+b)^n} \\
    \text{III. 不同的不可分解二次因子 }(ax^2+bx+c) &\quad \frac{Ax+B}{ax^2+bx+c} \\
    \text{IV. 重复的不可分解二次因子 } (ax^2+bx+c)^n &\quad \frac{A_1x+B_1}{ax^2+bx+c} + \frac{A_2x+B_2}{(ax^2+bx+c)^2} + \dots + \frac{A_n x+B_n}{(ax^2+bx+c)^n}
\end{array}
$$
