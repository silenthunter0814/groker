...
\textbf{1. 导数与不定积分的互逆关系}

不定积分与微分（求导数）是互逆运算。

不定积分的导数等于被积函数：$$\left(\int f(x) dx\right)' = f(x)$$

函数的微分（导数）的不定积分等于该函数与任意常数之和：$$\int F'(x) dx = F(x) + C$$
其中，$C$ 是积分常数。
...
基于微分的链式法则：
\begin{align}
    \int u'(v)\cdot v' \,dx = \int u'(v) \,dv = \int \,du(v) = u(v(x)) + C
\end{align}

\medskip

\begin{example} 计算积分 $\int (2\cos x + \sin x) \,dx$。
    \begin{align*}
        \int (2\cos x + \sin x) \,dx &= \int 2\cos x \,dx + \int \sin x \,dx \\
        &= 2\cdot \int \,d\sin x + \int \,d(-\cos x) \\
        &= (2\sin x + C_1) - (\cos x + C_2) \\
        &= 2\sin x - \cos x + C
    \end{align*}
\end{example}

在上面的计算中 $C_1$ 和 $C_2$ 被合并成了一个常数项 $C$，在实际的计算中，$C_1$ 和 $C_2$ 可以省略。

\medskip

\begin{example} 计算积分 $\int \tan x \,dx$。
    \begin{align*}
        \int \tan x \,dx &= \int \frac{\sin x}{\cos x} \,dx \\
        &= \int \frac{1}{\cos x} \,d(-\cos x) = -\int \frac{1}{\cos x} \,d\cos x
    \end{align*}

    对照链式法则公式：
    \begin{align*}
        &u' = \frac{1}{x} \qquad u = \ln x \qquad v = \cos x \\
        \implies &u'(v) = \frac{1}{\cos x} \qquad u(v) = \ln(\cos x)
    \end{align*}
    所以 $$\int \tan x \,dx = -\int \frac{1}{\cos x} \,d\cos x = -\int \,d\ln |\cos x| = -\ln |\cos x| + C$$
\end{example}
...
不定积分 $\int f(x) dx = F(x) + C$ 表示由函数 $f(x)$ 的所有原函数组成的曲线族。

对于曲线族中的任意一条曲线 $y = F(x) + C_1$ 和 $y = F(x) + C_2$，在横坐标 $x_0$ 处的切线斜率都等于 $f(x_0)$，因此它们是互相平行的。

这个曲线族中的任意一条曲线，都可以通过将其中任一条曲线沿 $y$ 轴方向作平移得到。
...
换元法用于简化积分变量，通过引入新变量 $u = g(x)$ ，使积分形式更简单。步骤：
...
\begin{example} 求积分 $\int \frac{1}{3 + 5 \cos x} \,dx$。
    ...
    代入万能换元公式：
    \begin{align*} 
        \int \frac{1}{3 + 5 \cos x} \,dx &= \int \frac{1}{3 + 5 \left(\frac{1 - t^2}{1 + t^2}\right)} \cdot \frac{2}{1 + t^2} \,dt \\ 
    ...
    替换回 $x$： 
    $$\int \frac{1}{3 + 5 \cos x} \,dx = \frac{1}{4} \ln \left| \frac{2 + \tan \frac{x}{2}}{2 - \tan \frac{x}{2}} \right| + C$$
\end{example}
