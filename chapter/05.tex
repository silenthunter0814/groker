...
\begin{example} 求积分：$$\int \frac{x^3 + 4x^2 - x + 1}{x(x^2 + 1)^2} \,dx$$
    ...
    展开并分组：
    $$x^3 + 4x^2 - x + 1 = A(x^4 + 2x^2 + 1) + (Bx^2 + Cx)(x^2 + 1) + Dx^2 + Ex$$
    $$x^3 + 4x^2 - x + 1 = Ax^4 + 2Ax^2 + A + Bx^4 + Bx^2 + Cx^3 + Cx + Dx^2 + Ex$$
    $$x^3 + 4x^2 - x + 1 = (A + B)x^4 + (C)x^3 + (2A + B + D)x^2 + (C + E)x + (A)$$
...
$$
    ...
    第一项 (线性因子)$$\int \frac{1}{x} \,dx = \mathbf{\ln|x|}$$
    第二项 (不同的二次因子)将分子拆开：
    \begin{align*}
    ...
    $$\frac{3}{2} \int \frac{1}{u^2} du = \frac{3}{2} \left(-\frac{1}{u}\right) = \mathbf{-\frac{3}{2(x^2 + 1)}}$$
