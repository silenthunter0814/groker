...
\begin{definition}{无穷大和无穷小}
    ...
    （2）对于任意实数 C，$\infty + C = \infty, \varepsilon + C = C$。

    （3） $\varepsilon$ 是最小的正数，$-\varepsilon$ 是最大的负数，$\varepsilon = 0^+, -\varepsilon = 0^-$。

    （4）对于 $n > 1$ 有，$\log_n\infty < \sqrt[n]{\infty} < \infty < \infty^n < n^\infty$
\end{definition}
