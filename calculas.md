...
导数不再是一个莱布尼兹定义的神秘符号 ddxF(x)，现在只是一个重新定义的点 x 位置的点微分的商，即 d F(x)/d x，和小学的除法运算没有什么不同，更强调计算结果是瞬时变化率或几何图形上的斜率。

微积分被重新定义，删除积分定理等内容，重新定义积分为微分的逆运算。具体变化为点积分简称为积分取代不定积分，保留定积分并重新命名为区间积分对应区间微分。
